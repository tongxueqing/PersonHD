...
def produce_ma_mask(kp_array, img_size=(128, 64), point_radius=4):
    ...
    for f, t in limbs:
        ...
        norm_vec = point_radius * norm_vec / (np.linalg.norm(norm_vec)+1e-10)
        vetexes = np.array([
            kp_array[f] + norm_vec,
            kp_array[f] - norm_vec,
            kp_array[t] - norm_vec,
...
def load_generated_images(generated, gt, ):
    ...
    for file in tqdm.tqdm(os.listdir(generated)):
        if not file.endswith('.jpg'):
            continue
        gntimg = cv2.imread(os.path.join(generated,file))
        gntimg = cv2.resize(gntimg,(512,512))
        # gtimg= imread(os.path.join(gt,file))
        # fs = file.split('_')
        gtimg= cv2.imread(os.path.join(gt,file))
        # fs = file.replace('__','_').split('_')
...
if __name__ == "__main__":
    ...
    print('load imgs...')
    stm, sgm = load_generated_images(args.distorated_path,args.gt_path,)

    
    print('calculate  SSIM metric...')
    
    structured_masked = masked_ssim_score(sgm, stm)
    print('SSIM metric:',structured_masked)
