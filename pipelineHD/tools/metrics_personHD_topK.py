...
class LPIPS():
    ...
    def calculate_from_disk(self, path_1, path_2,save_path,K,batch_size=64, verbose=False, sort=True):
        if sort:
        ...
        result_s=np.concatenate(result,axis=0).flatten()
        plt.hist(result_s)
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        plt.savefig(os.path.join(save_path,'a.png'))
        plt.close()
        x_topK_min=np.argsort(result_s)[:K]
        x_topK_max=np.argsort(result_s)[-K:]

        def save_in_folder(min_imgs,max_imgs,save_path,id,path_1,path_2):
            
            ## save img
            if not os.path.exists(os.path.join(save_path,'min',str(id))):
                os.makedirs(os.path.join(save_path,'min',str(id)))
            if not os.path.exists(os.path.join(save_path,'max',str(id))):
                os.makedirs(os.path.join(save_path,'max',str(id)))

            with open(os.path.join(save_path,f'min{id}.pkl'),'wb') as f:
                min_imgs_=[ i.split('/')[-1] for i in min_imgs]
                pickle.dump(min_imgs_,f)

            with open(os.path.join(save_path,f'max{id}.pkl'),'wb') as f:
                max_imgs_=[ i.split('/')[-1] for i in max_imgs]
                pickle.dump(max_imgs_,f)

            with open(os.path.join(save_path,'readme.md'),'a') as f:
                f.write(f"path1:{path_1}  path2:{path_2}\n")
            for i in min_imgs:
                shutil.copy(i.replace('fake_p2','vis').replace('__','.jpg__'),os.path.join(save_path,'min',str(id),i.split('/')[-1]))
                with open(os.path.join(save_path,'min',str(id),'readme.md'),'a') as f:
                    f.write("min_imgs:"+i+'\n')
            for i in max_imgs:
                shutil.copy(i.replace('fake_p2','vis').replace('__','.jpg__'),os.path.join(save_path,'max',str(id),i.split('/')[-1]))
                with open(os.path.join(save_path,'max',str(id),'readme.md'),'a') as f:
                    f.write("max_imgs:"+i+'\n')

        # save_in_folder(x_topK_min,x_topK_max,files_1,save_path,0,path_1,path_2)


        sub=np.array([int(i.split('/')[-1].split('_')[0]) for i in files_1][:n_used_imgs])
        
        print('lpips: %.4f'%distance)
        colors=['red','purple','teal','green','gold',"yellow", 'orange', 'gray','darkred', 'wheat','maroon', 'blue', 'darkgreen', 'pink', 'blueviolet', 'plum', 'lightgreen','brown', 'tan','navy']
        for i in range(81,101):
            x=(result_s[sub==i]).mean()
            plt.hist(result_s[sub==i],bins=10,density=True,alpha=0.5,color=colors[i-81],label=str(i))
        ...
        for i in range(81,101):
            files_1_=[files_1[_]   for _ in range(len(sub)) if sub[_]==i]
            y0=np.argsort(result_s[sub==i])
            y1=y0[:K//20]
            y2=y0[-K//20:]
            min_imgs=[files_1_[_] for _ in y1]
            max_imgs=[files_1_[_] for _ in y2]
            
            save_in_folder(min_imgs,max_imgs,save_path,i,path_1,path_2)
        return distance
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path',help='Path to save  topK',type=str)
    parser.add_argument('--K',help='topK',type=int,default=10)
    ...
    print('calculate lpips metric and topK...')
    lpips_score = lpips.calculate_from_disk(args.distorated_path, args.gt_path,args.save_path, args.K,sort=False)
