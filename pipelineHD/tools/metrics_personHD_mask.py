...
class LPIPS():
    ...
    def calculate_mask_lpips(self, distorated_path, fid_real_path,seg_path, batch_size=64, verbose=False,sort=False):
        ...
        for i in tqdm.tqdm(range(n_batches)):
            ...
            imgs_3 = np.array([imread(str(fn)).astype(np.float32) / 127.5 - 1 for fn in files_3[start:end]])
...
def load_generated_images(pose_pkl,size,generated, gt, annotation_file):
    ...
    for file in tqdm.tqdm(os.listdir(generated)):
        ...
        #kp_to (18,2)
        kp_to=poses[file]
        mask = produce_ma_mask(kp_to, size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pose_pkl', help='Path to pose pkl', type=str)
    ...
    lpips_masked = lpips.calculate_mask_lpips(args.distorated_path, args.fid_real_path,args.seg_path)
