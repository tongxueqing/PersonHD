...
from mmsr.data.util import (paired_paths_from_ann_file,paired_paths_from_ann_file3,
                            paired_paths_from_folder, paired_paths_from_lmdb)
from mmsr.utils import FileClient


class Ref_PersonHD_unsize_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(Ref_PersonHD_unsize_Dataset, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']

        self.in_folder, self.ref_folder,self.gt_folder = opt['dataroot_in'], opt[
            'dataroot_ref'],opt['dataroot_gt']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            print("self.gt_folder",self.gt_folder)
            self.paths = paired_paths_from_ann_file3(
                [self.in_folder, self.ref_folder,self.gt_folder], ['in', 'ref','gt'],
                self.opt['ann_file'])
        else:
    ...
    def __getitem__(self, index):
        ...
        img_in_gt = mmcv.imfrombytes(img_bytes).astype(np.float32) / 255.
        ...
        if scale>1:
            # bicubic upsample LR
            img_in_lq = img_in * 255
            img_in_lq = Image.fromarray(
                cv2.cvtColor(img_in_lq.astype(np.uint8), cv2.COLOR_BGR2RGB))
            ...
            img_in_up = img_in.copy()
            img_ref_lq = img_ref.copy()
        ...
        img_in, img_in_up, img_ref, img_ref_lq,  = totensor(  # noqa: E501
            [img_in,  img_in_up, img_ref, img_ref_lq],
            bgr2rgb=True,
            float32=True)
        ...
        if self.opt['phase'] != 'train':
            ...
            return_dict['img_origin'] = in_path
