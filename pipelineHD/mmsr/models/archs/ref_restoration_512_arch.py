...
class RestorationNet512(nn.Module):

    def __init__(self, ngf=64, n_blocks=16, groups=8,scale=2):
        super(RestorationNet512, self).__init__()
    ...
    def forward(self, x, pre_offset, img_ref_feat):
        ...
        # generate base: resize from 256*256 to 512*512
        base_x = F.interpolate(x, None, 2, 'bilinear', False)
        # resize from 256*256 to 128*128
        base= F.interpolate(x, scale_factor=0.5)

        #generate detail
        content_feat = self.content_extractor(base)
        upscale_restore = self.dyn_agg_restore(content_feat, pre_offset,
                                               img_ref_feat)
        return upscale_restore + base_x
